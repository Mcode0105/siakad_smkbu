<?php
session_start();
if (!isset($_SESSION['wali_kelas'])) {
    header("Location: ../login.php?ceklogin");
}
include_once '../template_walikelas/header.php';
include_once '../template_walikelas/sidebar.php';
include_once '../config/function.php';
$bulan = [
    "januari" => "januari",
    "februari" => "februari",
    "maret" => "maret",
    "april" => "april",
    "mei" => "mei",
    "juli" => "juli",
    "juni" => "juni",
    "agustus" => "agustus",
    "september" => "september",
    "oktober" => "oktober",
    "november" => "november",
    "desember" => "desember"
];


?>

<!-- Content Wrapper. Contains page content -->
<div class="content-wrapper">
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <h1>
            Input rekap Absensi

        </h1>
        <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>

            <li class="active">input rekap</li>
        </ol>
    </section>

    <!-- Main content -->
    <section class="content">

        <!-- Default box -->
        <div class="box">
            <div class="box-header with-border">
                <h3 class="box-title">Input Data</h3>
                <?php
                if (isset($_POST['edit'])) {
                    if (edit_absen_ganjil($_POST) > 0) {
                        echo "<div class='alert alert-success' role='alert'>
                    Data Berhasil Di ubah..!
                </div>";
                    }
                }

                ?>

                <div class="box-tools pull-right">
                    <button type="button" class="btn btn-box-tool" data-widget="collapse" data-toggle="tooltip"
                        title="Collapse">
                        <i class="fa fa-minus"></i></button>
                    <button type="button" class="btn btn-box-tool" data-widget="remove" data-toggle="tooltip"
                        title="Remove">
                        <i class="fa fa-times"></i></button>
                </div>
            </div>
            <?php
            $id = $_GET['id'];
            $result = mysqli_query($conn, "SELECT * FROM rekap_absen_ganjil WHERE id = $id ");
            $row    = mysqli_fetch_assoc($result);

            ?>
            <div class="box-body">
                <div class="row">
                    <div class="col-md-4">
                        <form role="form" method="post" action="">
                            <div class="box-body">
                                <div class="form-grup">
                                    <div class="col-md-6">
                                        <label for="exampleInputPassword1">Nisn</label>
                                        <input type="text" class="form-control" name="nisn" placeholder="nisn"
                                            value="<?= $row['nisn']; ?>">
                                        <input type="hidden" class="form-control" name="nama" placeholder="nisn"
                                            value="<?= $row['nama']; ?>">
                                        <input type="hidden" class="form-control" name="id" placeholder="nisn"
                                            value="<?= $row['id']; ?>">
                                                                                    <input type="hidden" class="form-control" name="kelas" placeholder="nisn"
                                            value="<?= $row['kelas']; ?>">
                                    </div>
                                </div>
                                <div class="form-grup">
                                    <div class="col-md-6">
                                        <label>Semester</label>
                                        <select name="semester" class="form-control">
                                            <option value=""><?= $row['semester'] ?></option>
                                            <option value="GENAP">GENAP</option>
                                            <option value="GANJIL">GANJIL</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-grup">
                                    <div class="col-md-6">
                                        <label>Bulan</label>
                                        <select name="bulan" class="form-control">
                                            <option><?= $row['bulan'] ?></option>
                                            <?php foreach ($bulan as $row) :  ?>
                                            <option value="<?= $row; ?>"><?= $row; ?></option>
                                            <?php endforeach ?>

                                        </select>
                                    </div>
                                </div>
                                <?php
                                $id = $_GET['id'];
                                $result = mysqli_query($conn, "SELECT * FROM rekap_absen_ganjil WHERE id = $id ");
                                $row = mysqli_fetch_assoc($result);
                                ?>
                                <div class="form-grup">
                                    <div class="col-md-6">
                                        <label>Tahun</label>
                                        <select name="tahun" class="form-control">
                                            <option><?= $row['tahun']; ?></option>
                                            <?php for ($i = 2000; $i <= 2030; $i++) : ?>
                                            <option value="<?= $i; ?>"><?= $i; ?></option>
                                            <?php endfor ?>
                                        </select>
                                    </div>
                                </div>
                                <?php
                                $id = $_GET['id'];
                                $result = mysqli_query($conn, "SELECT * FROM rekap_absen_ganjil WHERE id = $id ");
                                $row = mysqli_fetch_assoc($result);

                                ?>
                                <div style="text-align: center; font-size: 20px;" class="form-grup">
                                    <div class="col-md-12">
                                        <br>
                                        <label style="color: red;text-align: center;"
                                            for="exampleInputPassword1">Alfa</label>
                                    </div>
                                </div>
                                <div class="form-grup">
                                    <div class="col-md-6">
                                        <label for="exampleInputPassword1">Pagi</label>
                                        <input type="text" class="form-control" name="alfapagi" placeholder="pagi"
                                            value="<?= $row['alfapagi']; ?>">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <label for="exampleInputPassword1">Siang</label>
                                    <input type="text" class="form-control" name="alfasiang" placeholder="siang"
                                        value="<?= $row['alfasiang'] ?>">
                                </div>
                                <?php
                                $id = $_GET['id'];
                                $result = mysqli_query($conn, "SELECT * FROM rekap_absen_ganjil WHERE id = $id ");
                                $row = mysqli_fetch_assoc($result);

                                ?>
                                <div style="text-align: center" class="form-grup">
                                    <div class="col-md-12">
                                        <br>
                                        <label style="color: green; text-align: center; font-size: 20px; "
                                            for="exampleInputPassword1">Sakit</label>
                                    </div>
                                </div>
                                <div class="form-grup">
                                    <div class="col-md-6">
                                        <label for="exampleInputPassword1">Pagi</label>
                                        <input type="text" class="form-control" name="sakitpagi" placeholder="pagi"
                                            value="<?= $row['sakitpagi'] ?>">
                                    </div>
                                </div>

                                <div class="form-grup">
                                    <div class="col-md-6">
                                        <label for="exampleInputPassword1">Siang</label>
                                        <input type="text" class="form-control" name="sakitsiang" placeholder="siang"
                                            value="<?= $row['sakitsiang'] ?>">
                                    </div>
                                </div>
                                <div style="text-align: center" class="form-grup">
                                    <div class="col-md-12">
                                        <br>
                                        <label style="color: green; text-align: center; font-size: 20px; "
                                            for="exampleInputPassword1">Izin</label>
                                    </div>
                                </div>
                                <?php
                                $id = $_GET['id'];
                                $result = mysqli_query($conn, "SELECT * FROM rekap_absen_ganjil WHERE id = $id ");
                                $row = mysqli_fetch_assoc($result);
                                ?>
                                <div class="form-grup">
                                    <div class="col-md-6">
                                        <label for="exampleInputPassword1">Pagi</label>
                                        <input type="text" class="form-control" name="izinpagi" placeholder="pagi"
                                            value="<?= $row['izinpagi'] ?>">
                                    </div>
                                </div>

                                <div class="form-grup">
                                    <div class="col-md-6">
                                        <label for="exampleInputPassword1">Siang</label>
                                        <input type="text" class="form-control" name="izinsiang" placeholder="siang"
                                            value="<?= $row['izinsiang'] ?>">
                                    </div>
                                </div>
                            </div>

                    </div>
                    <!-- /.box-body -->
                    <div style="margin-left: 100px" class="col-md-12">
                        <div class="box-footer">
                            <button type="submit" name="edit" class="btn btn-primary">Submit</button>
                        </div>
                    </div>
                    </form>

                </div>

            </div>



            <!-- end -->
        </div>
        <!-- /.box-body -->
        <div class="box-footer">
            <!-- Footer -->
        </div>
        <!-- /.box-footer-->
</div>
<!-- /.box -->

</section>
<!-- /.content -->
</div>
<!-- /.content-wrapper -->

<?php
include_once '../template_walikelas/footer.php';
?>